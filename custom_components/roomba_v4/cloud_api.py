...
DISCOVERY_URL_TEMPLATE = "https://disc-prod.iot.irobotapi.com/v1/discover/endpoints?country_code={country_code}"
...
class IRobotCloudApi:
    ...
    async def discover_endpoints(self) -> dict[str, Any]:
        url = DISCOVERY_URL_TEMPLATE.format(
            country_code=urllib.parse.quote(self.country_code)
        )

        async with self.session.get(
            url, headers={"User-Agent": API_USER_AGENT}
        ) as resp:
            txt = await resp.text()
            if resp.status != 200:
                raise CloudApiError(
                    f"Endpoint discovery failed: {url} -> ({resp.status}) {txt[:400]}"
                )
        ...
        async with self.session.post(
            ...
            try:
                ...
                raise AuthenticationError(
                    f"Invalid Gigya account info JSON: {txt[:400]}"
                ) from err
    ...
    async def login_gigya(self, api_key: str) -> dict[str, Any]:
        ...
        if not gigya:
            raise AuthenticationError(
                f"Discovery response missing gigya config: keys={list(endpoints.keys())}"
            )
    ...
    async def login_irobot(self) -> dict[str, Any]:
        ...
        async with self.session.post(
            ...
            try:
                ...
                raise AuthenticationError(
                    f"Invalid iRobot login JSON: {txt[:400]}"
                ) from err
        if data.get("errorCode"):
            message = str(data.get("errorMessage") or data)
            if "No mqtt slot available" in message:
                self._mqtt_slot_blocked_until = datetime.now(tz=UTC) + timedelta(
                    minutes=5
                )
                try:
                    await self._write_runtime_debug(
                        "mqtt_slot_blocked",
                        {
    ...
    async def authenticate(self) -> dict[str, Any]:
        if (
            self._mqtt_slot_blocked_until
            and datetime.now(tz=UTC) < self._mqtt_slot_blocked_until
        ):
            raise AuthenticationError(
                f"No mqtt slot available; cooling down until {self._mqtt_slot_blocked_until.isoformat()}"
            )
        endpoints = await self.discover_endpoints()
        gigya = endpoints.get("gigya") or self.deployment.get("gigya")
        if not gigya:
            raise AuthenticationError(
                f"Discovery response does not contain gigya config. Keys: {list(endpoints.keys())}"
            )
    ...
    async def _aws_request(
        ...
        *,
        ...
        final_url = (
            final_url if not params else f"{final_url}?{urllib.parse.urlencode(params)}"
        )
        request_coro = (
            self.session.get if method.upper() == "GET" else self.session.post
        )
        ...
        try:
            async with request_coro(final_url, **kwargs) as resp:
                ...
                if resp.status == 403 and retry:
                    _LOGGER.debug(
                        "roomba_v4 AWS request got 403, re-authenticating: method=%s url=%s",
                        method,
                        url,
                    )
                ...
                if resp.status != 200:
                    _LOGGER.warning(
                        "roomba_v4 AWS request failed: method=%s status=%s url=%s body=%s",
                        method,
                        resp.status,
                        url,
                        txt[:500],
                    ...
                    raise CloudApiError(
                        f"AWS request failed ({resp.status}) {url}: {txt[:500]}"
                    )
                if not txt:
                    return {}
                try:
            ...
            _LOGGER.debug(
                "roomba_v4 AWS request timed out: method=%s url=%s", method, url
            )
            raise CloudApiError(f"AWS request timed out for {url}") from err
        except aiohttp.ClientError as err:
            _LOGGER.warning(
                "roomba_v4 AWS client error: method=%s url=%s error=%s",
                method,
                url,
                err,
            )
            raise CloudApiError(
                f"AWS request transport error for {url}: {err}"
            ) from err
        ...
        *,
        ...
        try:
            async with request_coro(final_url, **kwargs) as resp:
                ...
                if resp.status == 403 and retry:
                    _LOGGER.debug(
                        "roomba_v4 AWS detailed request got 403, re-authenticating: method=%s url=%s",
                        method,
                        url,
                    )
                    await self.authenticate()
                    return await self._aws_request_detailed(
                        url,
                        params,
                        retry=False,
                        accept=accept,
            ...
            _LOGGER.debug(
                "roomba_v4 AWS detailed request timed out: method=%s url=%s",
                method,
                url,
            )
            raise CloudApiError(f"AWS request timed out for {url}") from err
            ...
            _LOGGER.warning(
                "roomba_v4 AWS detailed client error: method=%s url=%s error=%s",
                method,
                url,
                err,
            )
    ...
    async def _aws_json_request(
        ...
        *,
        ...
        async with self.session.request(
            method.upper(), final_url, headers=headers, data=payload.encode("utf-8")
        ) as resp:
            txt = await resp.text()
            try:
                data = json.loads(txt) if txt else {}
        ...
        async with self.session.get(final_url, headers=headers) as resp:
            ...
            if resp.status == 403 and retry:
                ...
                return await self._aws_request_bytes(
                    url, params, retry=False, accept=accept
                )
            if resp.status != 200:
                try:
                    ...
                    txt = ""
                raise CloudApiError(
                    f"AWS binary request failed ({resp.status}) {url}: {txt[:500]}"
                )
            return data
    ...
    async def prime_livemap_session(self, robot_id: str) -> dict[str, Any]:
        ...
        status, data, txt = await self._aws_request_detailed(
            visible_url, visible_params, method="GET"
        )
        ...
        if selected is None:
            ...
            if isinstance(live_p2maps, list):
                for item in live_p2maps:
                    if isinstance(item, dict) and item.get("p2map_id"):
                        selected = {
                            "p2map_id": item.get("p2map_id"),
                            "active_p2mapv_id": item.get("p2mapv_id"),
                        }
                        break

        selected_p2map_id = (
            selected.get("p2map_id") if isinstance(selected, dict) else None
        )
        if isinstance(selected_p2map_id, str) and selected_p2map_id:
            summary["selected_p2map_id"] = selected_p2map_id
            detail_url = (
                f"{self.deployment['httpBaseAuth']}/v1/p2maps/{selected_p2map_id}"
            )
            detail_status, detail_data, detail_txt = await self._aws_request_detailed(
            ...
            if isinstance(detail_data, dict):
                summary["selected_active_p2mapv_id"] = (
                    ...
                    or (
                        selected.get("active_p2mapv_id")
                        if isinstance(selected, dict)
                        else None
                    )
                    or (
                        selected.get("p2mapv_id")
                        if isinstance(selected, dict)
                        else None
                    )
                )
    ...
    async def get_p2map_routines(
        self, p2map_id: str, limit: int = 50
    ) -> list[dict[str, Any]]:
        return await self._aws_request(
    ...
    async def get_favorites(self) -> dict[str, Any]:
        return await self._aws_request(
            f"{self.deployment['httpBaseAuth']}/v1/user/favorites"
        )

    async def get_schedules(self) -> dict[str, Any]:
        return await self._aws_request(
            f"{self.deployment['httpBaseAuth']}/v1/user/automations"
        )
    ...
    def _all_token_sources(self) -> dict[str, str]:
        ...
        for root_name, obj in (
            (
                "connection_tokens",
                (self.last_login_response or {}).get("connection_tokens") or {},
            ),
            (
                "live_activity_tokens",
                (self.last_login_response or {}).get("live_activity_tokens") or {},
            ),
    ...
    def _exact_query_and_header_variants(
        self, robot_id: str
    ) -> tuple[list[tuple[str, dict[str, str]]], list[tuple[str, Headers]], list[str]]:
    ...
    def _connection_token_query_variants(self) -> list[tuple[str, dict[str, str]]]:
        ...
        for authorizer in authorizer_names:
            variants.append(
                ("authorizer_only", {"X-Amz-CustomAuthorizer-Name": authorizer})
            )
            variants.append(
                ("authorizer_only_lower", {"x-amz-customauthorizer-name": authorizer})
            )
    ...
    def _aws_iot_presigned_wss_url(
        ...
        *,
        ...
        netloc = endpoint + (
            ":443" if include_port_in_url and ":" not in endpoint else ""
        )
    ...
    def _mqtt_connect_packet(
        ...
        variable = (
            self._mqtt_encode_utf8(proto_name)
            + bytes([protocol_level, connect_flags])
            + int(keepalive).to_bytes(2, "big")
        )
        return (
            bytes([0x10])
            + self._mqtt_encode_remaining_length(len(variable) + len(payload))
            + variable
            + payload
        )
    ...
    async def _write_packet_hex_trace(self, name: str, payload: dict[str, Any]) -> None:
        try:
            ...
            _LOGGER.debug(
                "v118 packet hex trace write failed for %s", name, exc_info=True
            )

    def _build_custom_authorizer_usernames(
        self, robot_id: str
        ...
        items.append(
            (
                "apk_style_username_query",
                username,
                None,
                ["iot_authorizer_name", "iot_signature", "iot_token"],
            )
        )
        return items

    async def _publish_via_tls_mqtt_custom_authorizer(
        self,
        *,
        endpoint: str,
        robot_id: str,
        ...
        for uname_name, username, password, key_names in usernames:
            for protocol_level in (4, 3):
                for client_id in filtered_client_ids:
                    info = {
                        "transport": "tls_mqtt_custom_authorizer",
                        "client_id": client_id,
                        "username_variant": uname_name,
                        "username_preview": username[:120],
                        "password": ("<set>" if password else None),
                        "token_keys": key_names,
                        "protocol_level": protocol_level,
                    }
                    _LOGGER.debug(
                        "v60 TLS MQTT attempt client_id=%s username_variant=%s proto=%s",
                        client_id,
                        uname_name,
                        protocol_level,
                    )
                    try:
                        reader, writer = await asyncio.wait_for(
                            asyncio.open_connection(
                                endpoint, 443, ssl=ssl_context, server_hostname=endpoint
                            ),
                            timeout=8.0,
                        )
                        try:
                            _LOGGER.debug(
                                "v60 TLS MQTT connected client_id=%s username_variant=%s proto=%s",
                                client_id,
                                uname_name,
                                protocol_level,
                            )
                            connect_packet = self._mqtt_connect_packet(
                                client_id,
                                username=username,
                                password=password,
                                protocol_level=protocol_level,
                            ...
                            _LOGGER.debug(
                                "v60 CONNECT HEX client_id=%s username_variant=%s proto=%s hex=%s",
                                client_id,
                                uname_name,
                                protocol_level,
                                self._hex_bytes(connect_packet),
                            )
                            writer.write(connect_packet)
                            await writer.drain()
                            _LOGGER.debug(
                                "v60 TLS MQTT CONNECT sent client_id=%s username_variant=%s proto=%s",
                                client_id,
                                uname_name,
                                protocol_level,
                            )
                            ...
                            _LOGGER.debug(
                                "v60 MQTT first_bytes client_id=%s username_variant=%s proto=%s len=%s hex=%s",
                                client_id,
                                uname_name,
                                protocol_level,
                                len(first),
                                self._hex_bytes(first),
                            )
                            _LOGGER.debug(
                                "v60 MQTT extra_bytes client_id=%s username_variant=%s proto=%s len=%s hex=%s",
                                client_id,
                                uname_name,
                                protocol_level,
                                len(extra),
                                self._hex_bytes(extra),
                            )
                            if len(first) >= 4 and first[0] == 0x20:
                                ...
                                if rc == 0:
                                    _LOGGER.debug(
                                        "v60 MQTT CONNACK success client_id=%s username_variant=%s proto=%s",
                                        client_id,
                                        uname_name,
                                        protocol_level,
                                    )
                                    ...
                                    writer.write(
                                        self._mqtt_publish_packet(topic, payload_bytes)
                                    )
                                    await writer.drain()
                                    return {
                                        "status": "published",
                                        ...
                                        "first_hex": self._hex_bytes(first),
                                        "extra_hex": self._hex_bytes(extra),
                                    }
                            ...
                            _LOGGER.debug(
                                "v60 TLS MQTT rejected client_id=%s username_variant=%s proto=%s reason=%s",
                                client_id,
                                uname_name,
                                protocol_level,
                                info.get("error"),
                        ...
                        attempts.append(info)
                        _LOGGER.debug(
                            "v60 TLS MQTT exception client_id=%s username_variant=%s proto=%s error=%r",
                            client_id,
                            uname_name,
                            protocol_level,
                            err,
        ...
        await self._write_runtime_debug(
            "tls_mqtt_custom_authorizer_failed", {"attempts": attempts[:40]}
        )
        return None
    ...
    def _mqtt_publish_packet(self, topic: str, payload: bytes) -> bytes:
        ...
        return (
            bytes([0x30])
            + self._mqtt_encode_remaining_length(remaining)
            + variable
            + payload
        )

    def _mqtt_subscribe_packet(
        self, packet_id: int, topics: list[str], qos: int = 1
    ) -> bytes:
    ...
    async def _mqtt_read_packet(self, websocket) -> dict[str, Any]:
        ...
        result = {
            ...
            "raw_hex": data.hex(),
            "raw_len": len(data),
            "session_present": bool(len(data) > 2 and data[2] & 0x01)
            if packet_type == 2
            else None,
    ...
    async def _mqtt_read_stream_packet(
        ...
        result = {
            ...
            "raw_hex": data.hex(),
            "raw_len": len(data),
            "session_present": bool(len(data) > 3 and data[2] & 0x01)
            if packet_type == 2
            else None,
            "return_code": data[3] if packet_type == 2 and len(data) > 3 else None,
        }
    ...
    async def _try_local_mqtt_subscriber(self, robot_id: str, cycle: int) -> bool:
        ...
        topics = ["#"]
        client_ids = [
            f"HA-local-{self.config['deviceId'][:8]}",
            ...
            for username in username_variants:
                for client_id in client_ids:
                    ...
                    try:
                        ...
                        writer.write(
                            self._mqtt_connect_packet(
                                client_id, username=username, password=mqtt_password
                            )
                        )
                        await writer.drain()
                        connack = await asyncio.wait_for(
                            self._mqtt_read_stream_packet(reader), timeout=5
                        )
                        ...
                        if connack.get("type") != 2 or connack.get(
                            "return_code"
                        ) not in {0, None}:
                        ...
                        await self._write_runtime_debug(
                            "local_subscriber_connected", attempt
                        )
                        ...
                        while not self._subscriber_stop.is_set():
                            try:
                                packet = await asyncio.wait_for(
                                    self._mqtt_read_stream_packet(reader), timeout=20
                                )
        ...
        await self._write_runtime_debug(
            "local_subscriber_attempts",
            {"cycle": cycle, "connected": False, "attempts": attempts[:80]},
        )
        return False
    ...
    def _preferred_payload_order(self, commanddef: dict[str, Any]) -> list[str]:
        ...
        base = [
            "raw_commanddef_payload",
            "robot_command_envelope",
            "commanddef_wrapped",
            "apk_commanddefs_upper",
            "apk_commanddefs_lower",
    ...
    def _filter_fragment_for_topic(
        self, fragment: dict[str, Any], mode: str
    ) -> dict[str, Any]:
        ...
        if mode == "stats":
            return {
                k: v for k, v in fragment.items() if k in {"batPct", "signal", "bin"}
            }
        if mode == "livemap":
            return {
                k: v
                for k, v in fragment.items()
                if k
                in {"cleanMissionStatus", "pose", "dock", "batPct", "signal", "bin"}
            }
        if mode == "shadow":
    ...
    def _extract_livemap_fragment(self, payload: Any) -> dict[str, Any]:
        ...
        def parse_cur_path(values: Any) -> dict[str, Any]:
            ...
            if (
                body
                and isinstance(body[-1], (int, float))
                and not isinstance(body[-1], bool)
                and body[-1] > 1_000_000_000
            ):
            ...
            return {
                ...
                "livemap": {
                    ...
                    **(
                        {"path_timestamp": trailing_timestamp}
                        if trailing_timestamp is not None
                        else {}
                    ),
                    **(
                        {"update_expire_ts": payload.get("update_expire_ts")}
                        if isinstance(payload.get("update_expire_ts"), (int, float))
                        else {}
                    ),
                    **(
        ...
        pos_update = (
            payload.get("pos_update")
            if isinstance(payload.get("pos_update"), dict)
            else None
        )
        ...
        if not pose_fragment:
            for item in candidates:
                ...
                point = (
                    pose.get("point")
                    if isinstance(pose, dict) and isinstance(pose.get("point"), dict)
                    else None
                )
                x = first_number(
                    (point or {}).get("x"),
        ...
        for item in candidates:
            cms = (
                item.get("cleanMissionStatus")
                if isinstance(item.get("cleanMissionStatus"), dict)
                else None
            )
            if cms:
                mission_fragment = {
                    "cleanMissionStatus": {k: cms.get(k) for k in cms_keys if k in cms}
                }
                break
            if any(key in item for key in cms_keys):
                mission_fragment = {
                    "cleanMissionStatus": {
                        k: item.get(k) for k in cms_keys if k in item
                    }
                }
                break
    ...
    def _build_mqtt_payload_variants(
        self, commanddef: dict[str, Any]
        ...
        variants: list[tuple[str, dict[str, Any]]] = [
            ...
            (
                "robot_command_envelope",
                {
                    ...
                    **(
                        {"ordered": int(commanddef.get("ordered"))}
                    ...
                    **(
                        {"select_all": bool(commanddef.get("select_all"))}
                        if "select_all" in commanddef
                        else {}
                    ),
                },
    ...
    def _is_room_clean_commanddef(self, commanddef: dict[str, Any]) -> bool:
        regions = commanddef.get("regions")
        return bool(
            commanddef.get("command") == "start"
            and commanddef.get("select_all") is False
            and isinstance(regions, list)
            and len(regions) > 0
        )

    def _room_clean_single_publish_plan(
        self, commanddef: dict[str, Any], irbt_topics: str, robot_id: str
        ...
        if forced_variant and forced_variant in by_name:
            ...
            selected = next(
                ((name, by_name[name]) for name in preferred_order if name in by_name),
                payload_variants[:1][0],
            )
    ...
    def _compose_effective_live_state(self) -> dict[str, Any]:
        ...
        if isinstance(stats, dict):
            allowed_stats = {
                k: v for k, v in stats.items() if k in {"batPct", "signal", "bin"}
            }
        ...
        meta["source_priority"] = [
            name
            for name in merge_order
            if isinstance(self._live_state_sources.get(name), dict)
            and self._live_state_sources.get(name)
        ]
        state["_meta"] = meta
        return state
    ...
    def _extract_shadow_state_fragments(
        self, payload: Any
    ) -> list[tuple[str, dict[str, Any]]]:
    ...
    def _derive_live_state_summary(self, state: dict[str, Any]) -> dict[str, Any]:
        cms = (
            state.get("cleanMissionStatus")
            if isinstance(state.get("cleanMissionStatus"), dict)
            else {}
        )
        ...
        return {
            ...
            "last_topic": (
                (state.get("_meta") or {}).get("last_topic")
    ...
    async def _ingest_live_state_payload(self, topic: str | None, payload: Any) -> None:
        ...
        if mode == "software":
            self._live_state.setdefault("_debug", {})["last_software_payload"] = (
        ...
        self._live_state["_meta"] = {
            ...
            "has_currentstate_update": bool(
                self._live_state_sources.get("currentstate_update")
            ),
        }
    ...
    async def _write_runtime_debug(self, stage: str, data: dict[str, Any]) -> str:
        ...
        if self.debug_dir:
            try:
                ...
                _LOGGER.debug(
                    "roomba_v4 runtime debug write failed stage=%s err=%s", stage, err
                )
    ...
    async def _append_event_packet(self, packet: dict[str, Any]) -> None:
        ...
        def _write() -> None:
            ...
            packets = packets[-400:]
            path.write_text(
                json.dumps(packets, indent=2, default=str), encoding="utf-8"
            )
            last_path.write_text(
                json.dumps(packet, indent=2, default=str), encoding="utf-8"
            )
            if packet.get("type") == "message" and isinstance(
                packet.get("payload_json"), dict
            ):
        ...
        if packet.get("type") == "message":
            await self._ingest_live_state_payload(
                packet.get("topic"), packet.get("payload_json")
            )
    ...
    def _cloud_topic_sequences(self, robot_id: str) -> list[list[str]]:
        ...
        irbt = str(
            robot.get("irbtTopics")
            or self.deployment.get("irbtTopics")
            or f"{svc}-irbthbu"
        )
    ...
    def _cloud_get_topics(self, robot_id: str) -> list[str]:
        ...
        topics = [
            f"$aws/things/{robot_id}/shadow/name/{name}/get" for name in preferred_names
        ]
    ...
    async def _capture_post_connack_idle(
        self, ws, *, seconds: float, phase: str
    ) -> list[dict[str, Any]]:
        packets: list[dict[str, Any]] = []
        deadline = asyncio.get_running_loop().time() + seconds
        while (
            asyncio.get_running_loop().time() < deadline
            and not self._subscriber_stop.is_set()
        ):
            remaining = max(0.05, deadline - asyncio.get_running_loop().time())
            try:
                packet = await asyncio.wait_for(
                    self._mqtt_read_packet(ws), timeout=min(0.5, remaining)
                )
            except asyncio.TimeoutError:
                continue
            packets.append(packet)
            await self._append_event_packet(
                {
                    ...
                    "type": "message"
                    if packet.get("type") == 3
                    else f"packet_{packet.get('type')}",
                    "topic": packet.get("topic"),
    ...
    async def _send_safe_shadow_gets(
        self, ws, *, robot_id: str, phase: str
    ) -> list[dict[str, Any]]:
        ...
        for idx, topic in enumerate(self._cloud_get_topics(robot_id), start=1):
            ...
            await self._write_packet_hex_trace(
                "mqtt_packet_hex_trace",
                {
    ...
    async def _capture_safe_trigger_window(
        self, ws, *, robot_id: str
    ) -> dict[str, Any]:
        await asyncio.sleep(0.5)
        sent = await self._send_safe_shadow_gets(
            ws, robot_id=robot_id, phase="safe_shadow_trigger"
        )
        observed = await self._capture_post_connack_idle(
            ws, seconds=3.0, phase="post_safe_shadow_trigger"
        )
        return {"sent": sent, "observed": observed[-40:]}

    async def _mqtt_wait_for_suback(
        self, ws, *, packet_id: int, topics: list[str], timeout: float, phase: str
    ) -> dict[str, Any]:
        ...
        while (
            ...
            packet = await asyncio.wait_for(
                self._mqtt_read_packet(ws), timeout=remaining
            )
            ...
            self._merge_live_state_from_packet(packet)
        raise CloudApiError(
            f"timeout waiting for suback packet_id={packet_id} topics={topics}"
        )
            ...
            try:
                reader, writer = await asyncio.wait_for(
                    asyncio.open_connection(
                        ...
                        server_hostname=None if use_ssl else None,
                    ),
                    timeout=4,
                )
                writer.close()
    ...
    async def async_ensure_event_subscriber(self, robot_id: str) -> None:
        ...
        livemap_topic_missing = bool(
            livemap_topic and livemap_topic not in self._subscriber_topics_subscribed
        )
        await self._write_runtime_debug(
            "mqtt_start",
            {
                ...
                "p2maps_priming_selected_p2map_id": (priming_summary or {}).get(
                    "selected_p2map_id"
                )
                if isinstance(priming_summary, dict)
                else None,
                "p2maps_priming_selected_active_p2mapv_id": (priming_summary or {}).get(
                    "selected_active_p2mapv_id"
                )
                if isinstance(priming_summary, dict)
                else None,
        ...
        if task and not task.done():
            await self._write_runtime_debug(
                "mqtt_restart_triggered",
                {
                    "robot_id": robot_id,
                    "reason": (
                        "livemap_topic_added"
                        if livemap_topic_missing
                        else "session_not_usable"
                    ),
                    "livemap_topic": livemap_topic,
                    "subscribed_topics": sorted(self._subscriber_topics_subscribed),
                },
        ...
        self._subscriber_task = asyncio.create_task(
            self._event_subscriber_loop(robot_id)
        )

    def _subscriber_connection_candidates(
        self, *, endpoint: str, robot_id: str, region: str
    ) -> list[dict[str, Any]]:
        _exact_query_variants, exact_header_variants, exact_client_ids = (
            self._exact_query_and_header_variants(robot_id)
        )
        ...
        client_ids = list(dict.fromkeys([cid for cid in preferred_client_ids if cid]))[
            :2
        ]
        if not client_ids:
        ...
        header_map = {
            name: headers
            for name, headers in exact_header_variants
            + self._custom_authorizer_header_variants()
        }
        apk_headers = header_map.get("apk_exact_headers") or Headers()
        combos: list[tuple[str, str, Headers, str]] = [
            (
                "direct_mqtt_always_on_subscriber",
                f"wss://{endpoint}:443/mqtt",
                apk_headers,
                "apk_exact_headers",
            ),
        ]
    ...
    async def _event_subscriber_loop(self, robot_id: str) -> None:
        ...
        endpoint = (
            self.deployment.get("mqttApp")
            or self.deployment.get("mqtt")
            or self.deployment.get("mqttAts")
        )
        if not endpoint:
            await self._log_event_status(
                {
        ...
        region = self.deployment.get("awsRegion") or self._region_from_endpoint(
            endpoint
        )
        ssl_context = await self._get_ssl_context()
        ...
        while not self._subscriber_stop.is_set():
            ...
            candidates = self._subscriber_connection_candidates(
                endpoint=endpoint, robot_id=robot_id, region=region
            )
            await self._write_runtime_debug(
                "subscriber_candidates",
                {
                    ...
                    "candidates": [
                        self._auth_debug_snapshot(candidate) for candidate in candidates
                    ],
                },
            )
                ...
                for connect_variant in connect_variants:
                    ...
                    attempt: dict[str, Any] = {
                        ...
                        "mqtt_username": (
                            "<empty>"
                            if mqtt_username == ""
                            else ("<set>" if mqtt_username is not None else None)
                        ),
                        "mqtt_password": (
                            "<set>" if mqtt_password is not None else None
                        ),
                        "phase": "starting",
                    ...
                    try:
                        ...
                        async with websockets.connect(
                            ...
                            attempt["connect_send_completed_at"] = datetime.now(
                                tz=UTC
                            ).isoformat()
                            connack = await asyncio.wait_for(
                                self._mqtt_read_packet(ws), timeout=6
                            )
                            attempt["connack"] = connack
                            ...
                            for idx, group in enumerate(topic_sequences, start=1):
                                packet_id += 1
                                subscribe_packet = self._mqtt_subscribe_packet(
                                    packet_id, group, qos=1
                                )
                                proof_payload = {
                                    "cycle": cycle,
                                    "client_id": client_id,
                                ...
                                await self._write_runtime_debug(
                                    "post_command_shadow_refresh_mode", proof_payload
                                )
                                await self._write_packet_hex_trace(
                                    "mqtt_packet_hex_trace", proof_payload
                                )
                                ...
                                if suback.get("type") != 9:
                                    raise CloudApiError(
                                        f"expected suback for {group}, got {suback}"
                                    )
                                self._subscriber_topics_subscribed.update(
                                    topic for topic in group if topic
                                )
                                await self._write_runtime_debug(
                                    "subscriber_session_topics",
                                    {
                                        "cycle": cycle,
                                        "client_id": client_id,
                                        "url_variant": candidate["url_name"],
                                        ...
                                        "subscribed_topics": sorted(
                                            self._subscriber_topics_subscribed
                                        ),
                            ...
                            warmup_packets = await self._capture_post_connack_idle(
                                ws, seconds=2.0, phase="post_subscribe_warmup"
                            )
                            for packet in warmup_packets:
                                if packet.get("topic"):
                                    topic_hits[packet["topic"]] = (
                                        topic_hits.get(packet["topic"], 0) + 1
                                    )
                                    ...
                                    first_messages.append(
                                        {
                                            ...
                                            "topic": packet.get("topic"),
                                            "payload_text_prefix": payload_text[:512]
                                            if isinstance(payload_text, str)
                                            else None,
                                            "payload_json": packet.get(
                                                "payload_json", packet
                                            ),
                                        }
                                    )
                            ...
                            attempt["steady_state_entered_at"] = datetime.now(
                                tz=UTC
                            ).isoformat()
                            await self._write_runtime_debug(
                            ...
                            while not self._subscriber_stop.is_set():
                                try:
                                    packet = await asyncio.wait_for(
                                        self._mqtt_read_packet(ws), timeout=8
                                    )
                                    idle_cycles = 0
                                except asyncio.TimeoutError:
                                    idle_cycles += 1
                                    if not bootstrap_sent and not self._live_state:
                                        ...
                                        attempt["safe_shadow_trigger"] = {
                                            "sent": safe_trigger_result.get("sent", [])[
                                                -10:
                                            ],
                                            "observed_count": len(
                                                safe_trigger_result.get("observed", [])
                                            ),
                                        }
                                        await self._write_runtime_debug(
                                            "safe_shadow_trigger",
                                        ...
                                        for packet in safe_trigger_result.get(
                                            "observed", []
                                        ):
                                            if packet.get("topic"):
                                                topic_hits[packet["topic"]] = (
                                                    topic_hits.get(packet["topic"], 0)
                                                    + 1
                                                )
                                        continue
            ...
            await self._write_runtime_debug(
                "subscriber_attempts",
                {"cycle": cycle, "connected": connected, "attempts": attempts[:100]},
            )
            if self._subscriber_stop.is_set():
                break
    ...
    async def _publish_via_app_like_wss(
        ...
        *,
        ...
        _, header_variants, exact_client_ids = self._exact_query_and_header_variants(
            robot_id
        )
        app_like_headers = [
            (name, headers)
            for name, headers in header_variants
            if name == "apk_exact_headers"
        ]
        ...
        preferred_client_ids = exact_client_ids + [
            cid for cid in client_ids if cid not in exact_client_ids
        ]
        for header_name, extra_headers in app_like_headers:
            for client_id in preferred_client_ids[:4]:
                ...
                _LOGGER.debug(
                    "v60 APK-like websocket attempt client_id=%s header_variant=%s",
                    client_id,
                    header_name,
                )
                try:
                    async with websockets.connect(
                        ...
                        if connack.get("type") != 2 or connack.get(
                            ...
                            _LOGGER.debug(
                                "v60 APK-like websocket rejected client_id=%s connack=%s",
                                client_id,
                                connack,
                            )
                            continue
                        if response_topics:
                            _LOGGER.debug(
                                "v60 APK-like websocket skipping pre-publish subscribe client_id=%s response_topics=%s",
                                client_id,
                                response_topics[:8],
                            )
                        observed_packets = []
                        for topic in topics:
                            for variant_name, body in payload_variants:
                                ...
                                _LOGGER.debug(
                                    "v60 APK-like websocket published client_id=%s topic=%s payload_variant=%s",
                                    client_id,
                                    topic,
                                    variant_name,
                                )
                                ...
                                while asyncio.get_running_loop().time() < end_time:
                                    ...
                                    try:
                                        ...
                                        observed_packets.append(
                                            {"error": repr(err), "during": variant_name}
                                        )
                                        break
                                    observed_packets.append(packet)
                                    if packet.get("type") == 3:
                                        _LOGGER.debug(
                                            "v60 APK-like websocket observed publish client_id=%s payload_variant=%s response_topic=%s payload=%s",
                                            client_id,
                                            variant_name,
                                            packet.get("topic"),
                                        ...
                                        _LOGGER.debug(
                                            "v60 APK-like websocket observed packet client_id=%s payload_variant=%s packet=%s",
                                            client_id,
                                            variant_name,
                                            packet,
                                        )
                                if observed_packets:
                                    await self._write_runtime_debug(
                                        "mqtt_observed_packets",
                                        {
                                            "client_id": client_id,
                                            "topic": topic,
                                            "payload_variant": variant_name,
                                            "observed_packets": observed_packets[-20:],
                                        },
                                    )
                        ...
                        return {
                            ...
                            "payload_variant": payload_variants[0][0]
                            if payload_variants
                            else None,
                            "observed_packets": observed_packets[-10:],
                        }
                    ...
                    _LOGGER.debug(
                        "v60 APK-like websocket failed client_id=%s header_variant=%s error=%r",
                        client_id,
                        header_name,
                        err,
                    )
    ...
    async def _publish_via_aws_iot_mqtt(
        self, commanddef: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        region = (
            self.deployment.get("awsRegion")
            or self.credentials["CognitoId"].split(":")[0]
        )
        irbt_topics = (
            self.deployment.get("irbtTopics")
            or f"{self.deployment.get('svcDeplId', 'v007')}-irbthbu"
        )
        topics = [
        ...
        if self._is_room_clean_commanddef(commanddef):
            topics, payload_variants = self._room_clean_single_publish_plan(
                commanddef, irbt_topics, robot_id
            )
            _LOGGER.debug(
                "v60 room-clean single-shot selected robot_id=%s topic=%s payload_variant=%s region_schema=%s",
                robot_id,
                topics[0],
                payload_variants[0][0],
                commanddef.get("_room_region_schema"),
            )
        else:
            by_name = {name: body for name, body in payload_variants}
            preferred = self._preferred_payload_order(commanddef)
            payload_variants = [
                (name, by_name[name]) for name in preferred if name in by_name
            ]
        _LOGGER.debug(
            "v60 MQTT publish start endpoint=%s robot_id=%s", endpoint, robot_id
        )
        startup_debug_path = await self._write_runtime_debug(
            "mqtt_start",
            {
                "endpoint": endpoint,
                "robot_id": robot_id,
                ...
                "exact_connection_token_bundle": {
                    k: (
                        v[:4] + "…" + v[-4:]
                        if isinstance(v, str) and len(v) > 12
                        else "<redacted>"
                    )
                    for k, v in self._exact_connection_token_bundle(robot_id).items()
                },
            },
        )
        ...
        path_variants = ["/mqtt"]
        exact_query_variants, exact_header_variants, exact_client_ids = (
            self._exact_query_and_header_variants(robot_id)
        )
                ...
                url_variants.append(
                    (
                        ...
                        self._aws_iot_presigned_wss_url(
                            endpoint, region=region, extra_query=auth_query, path=path
                        ),
                    )
                )
        ...
        for auth_name, auth_query in self._connection_token_query_variants():
            url_variants.append(
                (
                    ...
                    self._aws_iot_presigned_wss_url(
                        endpoint, region=region, extra_query=auth_query
                    ),
                )
            )
        ...
        header_variants = (
            exact_header_variants + self._custom_authorizer_header_variants()
        )
            ...
            for header_name, extra_headers in header_variants:
                for client_id in client_id_candidates:
                    conn_info: dict[str, Any] = {
                        "client_id": client_id,
                        "url_variant": url_name,
                        "header_variant": header_name,
                        **self._redact_url_query_keys(wss_url),
                    }
                    _LOGGER.debug(
                        "v60 MQTT websocket attempt client_id=%s url_variant=%s header_variant=%s",
                        client_id,
                        url_name,
                        header_name,
                    )
                    ...
                    try:
                        async with websockets.connect(
                            ...
                            _LOGGER.debug(
                                "v60 MQTT websocket opened client_id=%s url_variant=%s header_variant=%s",
                                client_id,
                                url_name,
                                header_name,
                            )
                            ...
                            for topic in topics:
                                for variant_name, body in payload_variants:
                                    ...
                                    attempt = {
                                        "topic": topic,
                                        "payload_variant": variant_name,
                                        "payload_preview": payload_bytes[:400].decode(
                                            "utf-8", errors="replace"
                                        ),
                                    }
                                    try:
                                        await ws.send(
                                            self._mqtt_publish_packet(
                                                topic, payload_bytes
                                            )
                                        )
                                        attempt["published"] = True
                                        try:
                                            maybe_reply = await asyncio.wait_for(
                                                self._mqtt_read_packet(ws), timeout=0.25
                                            )
                                    ...
                                    if attempt.get("published"):
                                        success = {
                                            ...
                                            "endpoint": endpoint,
                                            "region": region,
                                            "wss_url_redacted": self._redact_url_query_keys(
                                                wss_url
                                            ),
                                            "client_id": client_id,
                                            "topic": topic,
                                        ...
                                        _LOGGER.debug(
                                            "v60 MQTT publish success topic=%s payload_variant=%s debug=%s",
                                            topic,
                                            variant_name,
                                            startup_debug_path,
                                        )
                        ...
                        _LOGGER.debug(
                            "v60 MQTT websocket failed client_id=%s url_variant=%s header_variant=%s error=%s",
                            client_id,
                            url_name,
                            header_name,
                            err,
        ...
        failure_payload = {
            ...
            "url_variants": [name for name, _ in dedup_urls],
    ...
    def _subscriber_ws_is_usable(self, *, robot_id: str | None = None) -> bool:
        ...
        if (
            robot_id
            and self._subscriber_robot_id
            and self._subscriber_robot_id != robot_id
        ):
            return False
        task = self._subscriber_task
    ...
    async def _ensure_managed_command_session(
        self, robot_id: str, *, timeout: float = 20.0
    ) -> None:
        ...
        if not self._subscriber_ws_is_usable(robot_id=robot_id):
            raise CloudApiError(
                "managed MQTT subscriber session is not ready for command publish"
            )

    async def _publish_via_existing_subscriber_session(
        self, commanddef: dict[str, Any]
    ) -> dict[str, Any] | None:
        robot_id = str(commanddef.get("robot_id") or "")
        if not robot_id:
        ...
        if self._is_room_clean_commanddef(commanddef):
            ...
            by_name = {
                name: body
                for name, body in self._build_mqtt_payload_variants(commanddef)
            }
            preferred = self._preferred_payload_order(commanddef)
    ...
    async def _run_named_shadow_refresh_retry_window(
        ...
        *,
        ...
        refresh_topics = [
            topic
            for topic in self._cloud_get_topics(robot_id)
            if "/shadow/name/ro-currentstate/get" in topic
            or "/shadow/name/ro-stats/get" in topic
            or (include_generic_shadow and topic.endswith("/shadow/get"))
        ]
        ...
        for attempt_index, delay in enumerate(delays, start=1):
            ...
            for topic in refresh_topics:
                ...
                async with self._subscriber_send_lock:
                    if self._subscriber_ws is None or getattr(
                        self._subscriber_ws, "closed", False
                    ):
                        return {"sent": sent, "observed": observed}
                    await self._write_runtime_debug(
                        phase,
            ...
            observed.extend(
                await self._capture_post_connack_idle(
                    ws, seconds=capture_seconds, phase=phase
                )
            )
        return {"sent": sent[-20:], "observed": observed[-40:]}

    async def publish_commanddef_via_cloud_mqtt(
        self, commanddef: dict[str, Any]
    ) -> dict[str, Any]:
        robot_id = str(commanddef.get("robot_id") or "")
        _LOGGER.debug(
            "v60 publish_commanddef_via_cloud_mqtt entered robot_id=%s command=%s",
            robot_id,
            commanddef.get("command"),
        )
        async with self._command_send_lock:
            ...
            for attempt_no in (1, 2):
                try:
                    result = await self._publish_via_existing_subscriber_session(
                        commanddef
                    )
                    if result:
                        _LOGGER.debug(
                            "v60 publish_commanddef_via_cloud_mqtt completed status=%s transport=%s",
                            result.get("status"),
                    ...
                    if attempt_no == 1 and robot_id:
                        ...
                        await asyncio.wait_for(
                            self._subscriber_ready.wait(), timeout=20.0
                        )
                        continue
                    break
            raise CloudApiError(f"Managed MQTT command publish failed: {last_error}")

    async def async_send_simple_command(
    ...
    def get_cloud_transport_debug_info(self, robot_id: str) -> dict[str, Any]:
        ...
        if isinstance(robot, dict):
            for key, value in robot.items():
                if (
                    ...
                    or "iot" in key.lower()
                    or key.lower()
                    in {"password", "blid", "sku", "softwarever", "svcdeplid"}
                ):
        ...
        return {
            ...
            "login_response_keys": sorted(login.keys())
            if isinstance(login, dict)
            else [],
            "discovery_url_used": (self.discovered_endpoints or {}).get(
                "_discovery_url_used"
            ),
