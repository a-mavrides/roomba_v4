...
DISCOVERY_FALLBACK_COUNTRY_CODES = ("GB", "US")
...
class IRobotCloudApi:
    ...
    async def discover_endpoints(self) -> dict[str, Any]:
        ...
        configured_country = str(self.country_code or "").strip().upper()
        if configured_country:
            country_codes.append(configured_country)
        for fallback_country in DISCOVERY_FALLBACK_COUNTRY_CODES:
            if fallback_country not in country_codes:
                country_codes.append(fallback_country)

        urls = [
            DISCOVERY_URL_TEMPLATE.format(country_code=urllib.parse.quote(country_code))
        ...
        for url in urls:
            try:
                async with self.session.get(url, headers={"User-Agent": API_USER_AGENT}) as resp:
                    ...
                    if resp.status != 200:
                        ...
                        continue
                    data = json.loads(txt)
            except (aiohttp.ClientError, asyncio.TimeoutError, JSONDecodeError) as err:
            ...
            _LOGGER.debug(
                "roomba_v4 discovery succeeded: url=%s current_deployment=%s",
                url,
                current,
            )
            return data
